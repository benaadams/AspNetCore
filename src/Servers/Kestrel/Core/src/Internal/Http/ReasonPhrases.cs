...
namespace Microsoft.AspNetCore.Server.Kestrel.Core.Internal.Http
{
    internal static class ReasonPhrases
    {
        ...
        private static readonly Dictionary<int, byte[]> _definedStatusCodes = new Dictionary<int, byte[]>
        {
        ...
        public static byte[] ToStatusBytes(int statusCode, string reasonPhrase = null)
        {
            ...
            {
                if (_definedStatusCodes.TryGetValue(statusCode, out var reasonBytes))
                {
                    return reasonBytes;
                }
                else
                {
                    return GetUnknownStatusBytes(statusCode);
                }
            }

            return GetNonStandardStatusBytes(statusCode, reasonPhrase);
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        private static byte[] GetUnknownStatusBytes(int statusCode)
        {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static byte[] GetNonStandardStatusBytes(int statusCode, string reasonPhrase)
        {
            return Encoding.ASCII.GetBytes(statusCode.ToString(CultureInfo.InvariantCulture) + " " + reasonPhrase);
        }
    }
